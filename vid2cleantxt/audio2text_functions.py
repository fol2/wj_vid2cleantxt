...
def init_neuspell(model_name: str = "subwordbert", fallback_to_symspell: bool = True) -> any:
    """
    init_neuspell - initialize the neuspell spell checker with fallback to SymSpell

    Parameters
    ----------
    model_name : str, optional
        The name of the model to use, by default "subwordbert"
        Options: ["subwordbert", "bert", "roberta", "xlnet"]
    fallback_to_symspell : bool, optional
        Whether to fall back to SymSpell if NeuSpell fails, by default True

    Returns
    -------
    Any
        The spell checker object (either NeuSpell or SymSpell)
    """
    import logging
    import os
    import tempfile
    ...
    try:
        # Filter PyTorch future warning about weights_only
        ...
        # Validate model name
        valid_models = ["subwordbert", "bert", "roberta", "xlnet"]
        if model_name.lower() not in valid_models:
            logger.warning(f"Invalid model name {model_name}. Using subwordbert instead.")
            model_name = "subwordbert"
        
        # Set up model directories
        temp_dir = os.path.join(tempfile.gettempdir(), 'neuspell_models')
        os.makedirs(temp_dir, exist_ok=True)
        torch.hub.set_dir(temp_dir)

        try:
            # Initialize checker with weights_only=True for PyTorch loading
            checker = neuspell.BertChecker(model_name)
            checker.from_pretrained(
                map_location='cpu',
                cache_dir=temp_dir,
                weights_only=True  # Add this parameter to address the warning
            )
            
            # Verify model loaded correctly
            if not hasattr(checker, 'model') or checker.model is None:
                raise RuntimeError("Model failed to load properly")
            
            logger.info(f"Successfully loaded NeuSpell {model_name} model")
            return checker
            
        except Exception as e:
            if not fallback_to_symspell:
                raise
            
            logger.warning(f"Failed to initialize NeuSpell: {str(e)}")
            logger.info("Falling back to SymSpell checker")
            
            # Initialize SymSpell as fallback
        ...
        logger.warning(f"Unexpected error with NeuSpell: {str(e)}")
        logger.info("Falling back to SymSpell checker")
...
def neuspell_freetext(textlines, ns_checker=None, verbose=False):
    """
    neuspell_freetext - spell check text using either NeuSpell or SymSpell

    Parameters
    ----------
    textlines : list or str
        Text to be spell checked
    ns_checker : Union[neuspell.BertChecker, SymSpell], optional
        Spell checker object, by default None
    ...
    str
        The corrected text
    """
    checker = ns_checker or init_neuspell(fallback_to_symspell=True)
    textlines = list(textlines) if isinstance(textlines, str) else textlines
    corrected_list = []
    ...
    for line_obj in textlines:
        ...
        # Handle different checker types
        if isinstance(checker, neuspell.BertChecker):
            corrected = checker.correct_strings([line.lower()])
            corrected_text = " ".join(corrected)
        else:  # SymSpell
            corrected_text = symspell_freetext([line], speller=checker, verbose=verbose)

        corrected_list.append(corrected_text + "\n")

    corrected_text = corr(" ".join(corrected_list))
    
    if verbose:
        print(f"Finished correcting at: {datetime.now()}\n")
...
def spellcorrect_pipeline(
    ...
    """
    spellcorrect_pipeline - takes a filepath and filename and returns a corrected version of the text.
    
    Parameters
    ----------
    filepath : str
        The filepath to the file to be corrected
    filename : str
        The filename of the file to be corrected
    method : str, optional
        The method to use for spell checking. Options are "neuspell" or "symspell"
    spell_checker : Any, optional
        If None, will initialize a new object for the spell checker
    linebyline : bool, optional
        Whether to save the corrected text as a list of lines or a single string
    verbose : bool, optional
        Whether to print out the progress of the spellchecking process
    skip_correction : bool, optional
        If True, skips spell correction (for Whisper Large V3 or non-English)
    language : str, optional
        Language code of the text (e.g., "en", "no", "de")

    Returns
    -------
    dict
        The processed text and related data
    """
    with open(join(filepath, filename), "r", encoding="utf-8", errors="ignore") as file:
        textlines = file.readlines()

    if skip_correction or language.lower() != "en":
        # Skip spell correction but still do sentence boundary detection
        corrected_text = textlines
        loc_SC = "original_text"  # Change directory name to reflect no correction
    else:
        # Original spell correction logic
    ...
    # Rest of the processing (sentence boundary detection, etc.)
